...
async function init() {
    try {
        ...
        statusElement.textContent = 'Models loaded. Processing test images...';
        
        // Auto-run on test images
        await processSlot('./testdata/P1080390.JPG', 0);
        await processSlot('./testdata/P1080400.JPG', 1);
...
async function processSlot(imageUrl, slotIndex) {
    ...
    try {
        ...
        let w = img.width;
        let h = img.height;
        // Resize heavily for performance/memory if needed, or keep original if manageable.
        // The original code was dividing by 8 then * 8 likely for alignment, not resizing.
        // But large images 12MP+ might be too heavy for webgpu demo.
        // Let's assume we want to process them at reasonable size or original.
        // Usually SuperPoint works on resized ~640x480 or similar.
        // Let's constrain max dimension to say 1200 to be safe and fast.
        const maxDim = 1024;
